...
def compute_center_frames(
    ...
    # The same rule at the closest spacing the sampler allows, so there is no
    # pixel count to keep in step with the frame.
...
def run_hybrid_for_dataset(dataset_id, config, run_paths):
    ...
    # IMAGE_SIZE is the input image's true size, which nothing lays out by.
