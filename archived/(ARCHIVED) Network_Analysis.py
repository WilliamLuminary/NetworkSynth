...
def load_graph_fromcsv(cur_file=None, directed=False):
    ...
    G = nx.read_edgelist(
        ...
        delimiter=",",
        comments="Source",
        data=[("Weight", float), ("Length", float)],
    )  # pay attention to this, 'Length' is true weight, 'Weight' is the diameter
    ...
    if len(G.edges()) <= 20:
        warnings.warn(
            "A graph is ignored because it has only a few edges. \nLocatoin: "
            + cur_file
        )
        return None
...
root = (
    "/home/zhangpeiyu/llm/interpretability_llm/chung_man/For_Xiongye/results_combine/"
)
...
for i in range(len(G_list)):
    if weight_flag == "True":
        ntau = wnfd_nk(
            G_list[i], Q, weight=True, draw=False, fdigi=1
        )  # Peiyu: weight=None
    else:
...
plt.savefig(
    root + "weighted_{}_nspectrum.svg".format(kX_index), bbox_inches="tight", dpi=600
)
plt.close()
...
Q = [q / 100 for q in range(-2000, 2001, 10)]
wei_ntauls_list = np.load(
    root + "/div_ntauls_{}_{}.npy".format(kX_index, weight_flag), allow_pickle=True
)
colors = ["#F14040", "#FF8000", "#37AD6B", "#B177DE"]
for i in range(len(wei_ntauls_list)):
    print(name[i])
...
#     spine.set_color('black')
plt.savefig(
    root + "weighted_{}_ndimension.svg".format(kX_index), bbox_inches="tight", dpi=600
)
...
for G in tqdm(G_list):
    if weight_flag == "True":
        closeness_centrality = nx.closeness_centrality(
            G, distance=lambda u, v, d: 1 / d["weight"]
        ).values()
    else:
