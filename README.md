...
`dist` is a code only copy of this repository, published so another project can carry NetworkSynth as a git submodule without the sample data tracked on `main`. It is an orphan branch holding just the code, `requirements.txt`, `LICENSE` and a README of its own. Nothing is edited there; every commit is generated by the publish script.

```bash
scripts/publish_dist.sh [source-ref] [tag]     # defaults to HEAD, no tag, never pushes
```
...
A pre-release, which may sit on any branch:

```bash
git tag -a vX.Y.Z-dev -m "NetworkSynth vX.Y.Z-dev"
...
`.github/workflows/publish-dist.yml` does the work, and the dist tag is always derived from yours, so the two cannot drift apart. Consumers follow `dist`, so it only ever holds released trees.
...
DIST_BRANCH=dist-test scripts/publish_dist.sh HEAD
