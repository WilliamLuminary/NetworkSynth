...
`.github/workflows/publish-dist.yml` does the work, and the dist tag is always derived from yours, so the two cannot drift apart. The `-dev` suffix is the only switch: it picks the branch and waives the `main` check, and `dist-v*` tags never match the `v*` trigger, so a publish cannot set itself off again.
